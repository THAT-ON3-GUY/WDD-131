...
function search() {
    ...
    let filterRecipes = recipes.filter(function(recipe){
        return (
            ...
            recipe.tags.some(tag => tag.toLowerCase().includes(recipeQuery.toLowerCase()))
        );
    })
...
function recipeTemplate(recipe) {
    return `<div class="recipe-card">
        <img class="recipe-image" src="${recipe.image}" alt="${recipe.name}" fetchpriority="high" width="700" height="500">
        <div class="recipe-info">
            <p class="recipe-category">${tagTemplate(recipe.tags)}</p>
            <h2 class="amatic-sc-bold">${recipe.name}</h2>
